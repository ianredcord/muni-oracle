...
export default function TarotDrawClient() {
  const router = useRouter();
  const [phase, setPhase] = useState<DrawPhase>("initial");
  const [shuffledCards] = useState<TarotCard[]>(() => shuffleArray(TAROT_CARDS));
  ...
  const handleReveal = () => {
    ...
    setTimeout(() => {
      ...
      router.push(`/tarot/draw/result?card=${selectedCard.slug}`);
    }, 800);
  };
        ...
        <header className="text-center mb-8">
          ...
          >
            ...
            <p className="text-stone-500 text-base max-w-xl mx-auto leading-relaxed">
              {phase === "initial" && "在塔羅的原型智慧中，尋找你的療癒方向。請靜下心來，專注於您當下的感受。"}
              {phase === "shuffling" && "正在洗牌..."}
              {phase === "selecting" && "請憑直覺選取 1 張牌卡"}
